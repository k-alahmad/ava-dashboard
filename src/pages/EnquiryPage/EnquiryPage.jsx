...
import { ComposeColumns } from "./EnquiryListColumn";
import {
  toggleAllColumns,
  toggleColumn,
  exportToCSV,
...
const EnquiryPage = () => {
  ...
  const handleOnCSVExport = () => {
    exportToCSV("Enquiry", columns, enquiry);
  };
  return (
    <>
      <EnquiryDrawer
          ...
          <PageCard
            ...
            table={
              isLoading || isFetching ? (
                ...
                isSuccess && (
                  <AdminTable
                    noDataMessage="There are no Enquiry"
                    searchText={searchText}
                    columns={columns}
                    loading={isLoading}
                    data={enquiry?.entities}
                    dataCount={enquiry?.ids?.length}
                    onRowClick={(ev, row) => {
