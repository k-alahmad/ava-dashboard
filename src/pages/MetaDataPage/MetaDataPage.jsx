...
import { ComposeColumns } from "./MetaDataListColumns";
import {
  toggleAllColumns,
  toggleColumn,
  exportToCSV,
...
const MetaDataPage = () => {
  ...
  const handleOnCSVExport = () => {
    exportToCSV("meta-data", columns, metaData);
  };
  return (
    <>
      <MetaDataDrawer
          ...
          <PageCard
            ...
            table={
              isLoading || isFetching ? (
                ...
                <AdminTable
                  noDataMessage="There are no meta-data"
                  searchText={searchText}
                  columns={columns}
                  loading={isLoading}
                  data={metaData?.entities}
                  dataCount={metaData?.ids?.length}
                  onRowClick={(ev, row) => {
